...
public class MySQLSpecial extends AbstractDbSpecial {
    static {
        MySQLSpecial special = new MySQLSpecial();
        DbUtils.put(special.dbName(), special);
    }

    private static final String TEMPLATE_SQL = "insert ignore into barrier (trans_type, gid, branch_id, op, barrier_id, reason) values (?,?,?,?,?,?)";
